...
<core:if test="${sessionScope.loggedIn}">
	...
	<!-- success message -->
	<core:if test="${sessionScope.product_available_in_Stock == 'notexception'}">
	<!-- message -->
	...
	<!-- form row -->
		<div class="row justify-content-center ">
			<div class="col col-5 border border-success px-5  bg-white rounded">
				...
				<!--  form -->
				<form class="mb-5" action="addProductInStockLogic.jsp" method="post">
				  <div class="mb-3">
				    <label for="product_name_in_stock" class="form-label mt-3"><i class="bi bi-cart"></i> Product Name</label>
				    <input type="text" class="form-control" id="product_name_in_stock" name="productNameInStock" required placeholder="type Product name here">
				    </div>				  
				  
				  <div class="mb-3">
				    <label for="product_quantity_in_stock" class="form-label"><i class="fa-solid fa-scale-balanced"></i> Product Quantity (kg/L)</label>
				    <input type="text" oninput="this.value= this.value.replace(/[^\d.]/g, '')" class="form-control " id="product_quantity_in_stock" maxlength="65" name="productQuantityInStock" placeholder="type Product Quantity here" required>
				  </div>
...
<core:remove var="product_available_in_Stock"></core:remove>
